...
def future_value_of_flat(rate, nper, pv):
    return npf.fv(rate, nper, 0, -pv)

def monthly_payment(rate, nper, fv):
    return npf.pmt(rate, nper, 0, fv)

def chart_flat(months, monthly_rate,monthly_rate_deposit,pv,pm):
...
if __name__ == "__main__":
    fv = future_value_of_flat(rate, years, init_price)
    pm = monthly_payment(monthly_rate_deposit, months, fv)
    ...
    chart_flat(months, monthly_rate, monthly_rate_deposit, init_price, pm)
